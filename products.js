...
db.products.update({department: 'Hardware'}, {$inc: {price: 100}}, {multi: true});
...
db.products.find({stock: 0}, {_id: 0, name: 1}).pretty();


// 2. Find the stock count of all the products with a price below $100
db.products.find({price: {$lt: 100}}, {_id: 0, name: 1, price: 1, stock: 1});
...
db.products.find({color: 'red'}, {_id: 0, name: 1}).pretty();
...
//that either start with A and have a price of at least $100 or start with B
//and have a price of at most $100
db.products.find({
...
// 1. Find the total number of sales each department made and sort the
// results by the department name
db.products.aggregate([
  {$group: {_id: '$department', salesTotal: {$sum: '$sales'}}},
  {$sort: {_id: 1}}
]);
...
// 3. Find the number of out of stock products in each department and sort the
// results by the department name
db.products.aggregate([
  {$match: {stock: 0}},
  {$group: {_id: '$department', outOfStockTotal: {$sum: 1}}},
  {$sort: {_id: 1}}
]);
...
db.products.mapReduce(
  ...
  {
    query: {},
    out: 'number_of_products_per_color'
  }
).find();
...
db.products.mapReduce(
  ...
  {
    query: {},
    out: 'revenue_per_department'
  }
).find();


// 3. Find the potential revenue of each product (how much can each product
// make if the entire remaining stock is sold?)
db.products.mapReduce(
  function() {
...
db.products.mapReduce(
  function() {
    emit(this.name, (this.price * this.sales + this.price * this.stock));
  },
  function(key, values) {
    return Array.sum(values);
  },
